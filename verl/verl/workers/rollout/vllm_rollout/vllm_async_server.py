...
class vLLMHttpServerBase:
    ...
    def __init__(
        ...
        # Respect user-provided `rollout.max_model_len` (often needed to fit KV cache),
        # otherwise fall back to the model's max_position_embeddings.
        ...
        if self.node_rank == 0:
            ...
            # `get_free_port` returns a bound socket; close it immediately so vLLM can bind to the port.
            self._master_port, master_sock = get_free_port(self._server_address)
            master_sock.close()
            self._dp_master_port, dp_master_sock = get_free_port(self._server_address)
            dp_master_sock.close()
            logger.info(
