...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        if self.world_size > 1 and fsdp_version(self.actor.actor_module) == 1:
            handle = getattr(self.actor.actor_module, "_handle", None)
            if handle is not None and getattr(handle, "uses_sharded_strategy", True):
                handle.reshard(True)
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        if self.world_size > 1:
            if fsdp_version(self.ref_policy.actor_module) == 1:
                handle = getattr(self.ref_policy.actor_module, "_handle", None)
                if handle is not None and getattr(handle, "uses_sharded_strategy", True):
                    handle.reshard(True)
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        if self.world_size > 1 and fsdp_version(self.reward_module) == 1:
            handle = getattr(self.reward_module, "_handle", None)
            if handle is not None and getattr(handle, "uses_sharded_strategy", True):
                handle.reshard(True)
