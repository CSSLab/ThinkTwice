...
export WANDB_PROJECT=grpo-math
# offline mode
...
VERL_ROOT="$SCRIPT_DIR"
cd "$VERL_ROOT"

if [[ ! -f "$VERL_ROOT/recipe/dapo/main_dapo.py" ]]; then
    echo "Missing recipe submodule. Run: (cd \"$VERL_ROOT\" && git submodule update --init --recursive recipe)" >&2
    exit 1
fi
...
L=3000
GEN_B=96
OVERLONG_BUFFER_LEN=$((L / 4))
NAME=dapo_math_Qwen3-4B-Instruct-2507
...
CUDA_VISIBLE_DEVICES=${VISIBLE_DEVICES} PYTHONUNBUFFERED=1 python -m recipe.dapo.main_dapo \
    ...
    custom_reward_function.path="${VERL_ROOT}/verl/utils/reward_score/math_dataset.py" \
