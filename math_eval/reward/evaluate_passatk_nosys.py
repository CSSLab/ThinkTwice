...
# IMPORTANT: Import vllm FIRST before any sys.path modifications
...
# Import system prompt removal utility using importlib (no sys.path modification)
chat_template_path = VERL_DIR / "verl" / "utils" / "chat_template.py"
spec = importlib.util.spec_from_file_location("chat_template", chat_template_path)
chat_template_module = importlib.util.module_from_spec(spec)
sys.modules['chat_template'] = chat_template_module
spec.loader.exec_module(chat_template_module)
remove_system_prompt_from_text = chat_template_module.remove_system_prompt_from_text
...
def construct_reflection_prompts(tokenizer, raw_questions, base_responses, is_olmo3=False):
    prompts = []
    for question, answer in zip(raw_questions, base_responses):
        ...
        # Remove system prompt for OLMo3 models
        if is_olmo3:
            prompt = remove_system_prompt_from_text(prompt, tokenizer)
        prompts.append(prompt)
    return prompts


def run_benchmark(llm, tokenizer, boxed_reward_fn, bench_name, bench_path, num_samples, k_values, results, is_olmo3):
    ...
    for idx, row in df.iterrows():
        ...
        # Remove system prompt for OLMo3 models
        if is_olmo3:
            formatted_prompt = remove_system_prompt_from_text(formatted_prompt, tokenizer)
    ...
    for base_idx, m in enumerate(metadata):
        ...
        for sample_idx in range(num_samples):
            ...
            if not base_response:
                continue
            refl_prompts = construct_reflection_prompts(tokenizer, [raw_question], [base_response], is_olmo3=is_olmo3)
...
def main():
    ...
    # model_path = "/data1/models/LLM-Research/allenai/Olmo-3-7B-Instruct"
    # model_path = "/data1/qianfeng/ckpts/dapo_olmo3-7b-instruct-nosys/best_model"
    # model_path = "/data1/qianfeng/ckpts/grpo_olmo3-7b-instruct-nosys/best_model"
    model_path = "/data1/qianfeng/ckpts/drgrpo_olmo3-7b-instruct-nosys/best_model"
    # model_path = "/data1/qianfeng/ckpts/refpo_best_olmo3/best_model"
    ...
    # Detect if model is OLMo3 (needs system prompt removal)
    is_olmo3 = any(x in model_path.lower() for x in ['olmo', 'allenai/olmo'])

    print(f"Model: {model_path}")
    print(f"Num samples: {NUM_SAMPLES}")
    print(f"Pass@k values: {PASS_K_VALUES}")
    print(f"System prompt removal for OLMo3: {'ENABLED' if is_olmo3 else 'DISABLED'}")
    ...
    for bench_name, bench_path in BENCHMARKS.items():
        run_benchmark(llm, tokenizer, boxed_reward_fn, bench_name, bench_path, NUM_SAMPLES, PASS_K_VALUES, results, is_olmo3)

    output_path = REPO_ROOT / "math_eval" / "reward" / "passatk_posthoc_nosys_results.json"
